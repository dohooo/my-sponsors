...
const DOSO_LOGO = (width: number, y: number) =>
  readFileSync(path.resolve(__dirname, "./logo/doso.svg"), "utf-8")
    .replace("${x}", String((width - 80) / 2))
    .replace("${y}", String(y));
      ...
      composeAfter(compose, _, config) {
        if (
          ...
          compose
            ...
            .addSpan(10)
            .addRaw(DOSO_LOGO(config.width!, compose.height))
            .addSpan(100);
        }
      },
    },
